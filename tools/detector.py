...
def wrap(config, dic, img, box):
    ...
    # H, _ = cv2.findHomography(box, dst, cv2.RANSAC, 5.0)
    ...
    x_list = [round(xmin + i*x_) for i in range(4)]
    y_list = [round(ymin + i*y_) for i in range(4)]
...
def detect(config, dic, img, verify_dir=None):
    ...
    for cls, marker in enumerate(markers):
        ...
        if id:
            ...
            continue
        if verify_dir:
            cv2.circle(canvas, (round(lr[0]), round(lr[1])), int(0.004*h), (0, 0, 255), -1)
            count = 0
            for i, j in re_info['marker_points'].tolist():
                cv2.circle(canvas, (round(i), round(j)), int(0.006*h), (0, 255, 0), -1)
                cv2.putText(canvas, str(count), (round(i), round(j)), cv2.FONT_HERSHEY_SIMPLEX, round(round(0.004*h)),  (0, 255, 0), round(0.003*h), cv2.LINE_AA)
                count += 1
            cv2.drawContours(canvas, [np.int0(box)], 0, (0, 0, 255), 1)
            cv2.drawContours(canvas, [np.int0(expand_box)], 0, (0, 0, 255), 1)
            cv2.circle(canvas, (round(np.mean(box[:, [0]])), round(np.mean(box[:, [1]]))), round(round(0.006*h)), (255, 0, 0), -1)
            cv2.putText(canvas, str(id), (round(np.mean(box[:, [0]])), round(np.mean(box[:, [1]]))), cv2.FONT_HERSHEY_SIMPLEX, round(round(0.003*h)),  (255, 0, 0), round(0.003*h), cv2.LINE_AA)
    det_dic.update({
        'canvas': canvas,
    })
    return det_dic
...
def main(config, dic, src, verify_dir):
    ...
    if im_p[0][-4:] == '.jpg':
        for i in range(nums):
            cv2.imwrite(osp.join(verify_dir, path_to_name(im_p[i])), frames[i])
    else:
